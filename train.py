...
def main():
    ...
    if args.launcher == 'none':  # disabled distributed training  不用分布式训练
        opt['dist'] = False
        rank = -1
    ...
    # convert to NoneDict, which returns None for missing keys    NoneDict在字典找不到键的时候返回none，用来处理缺失键
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            train_size = int(math.ceil(len(train_set) / dataset_opt['batch_size']))# 向上舍入最近进的整数
    ...
    #### resume training    resume_state是一个字典，包含模型的训练状态，包括epoch，iter
    if resume_state:
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if rank <= 0 and opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                ...
                for val_data in val_loader:
                    ...
                    visuals = model.get_current_visuals()  # 可视化 
                    if which_model =="RCAN":
